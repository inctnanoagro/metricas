...
OUTPUT_NAME="${1:-metricas-clean-${TIMESTAMP}}"
...
if tar --version 2>&1 | grep -q "GNU tar"; then
    ...
    tar --no-xattrs \
        ...
        --exclude='.DS_Store' \
        --exclude='__MACOSX' \
        -czf "${PROJECT_ROOT}/${OUTPUT_NAME}.tar.gz" \
        "$PROJECT_NAME"
else
...
# Show results
OUTPUT_FILE="${PROJECT_ROOT}/${OUTPUT_NAME}.tar.gz"
...
TOTAL_UNWANTED=$((APPLEDOUBLE_COUNT + DS_STORE_COUNT + PYCACHE_COUNT + VENV_COUNT + MACOSX_COUNT))
...
if [ "$TOTAL_UNWANTED" -eq 0 ]; then
    ...
    tar -tzf "$OUTPUT_FILE" | grep -E '(\._|\.DS_Store|__pycache__|/venv/|__MACOSX)' | head -10
    echo
